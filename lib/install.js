...
function install(args, cb) {
  ...
  var meanieModule = args;

  //Array of modules given
  if (Array.isArray(meanieModule)) {
    return installSeries.call(this, meanieModule, cb);
  }
  ...
  //Clone
  cloneRepo(meanieModule, function(error, tmpDir) {
    ...
    copyFiles(tmpDir, self.projectDir, function(error) {
      ...
      //Add to config
      //TODO module version
      meanieFile.addModule(self.projectDir, meanieModule, '1.0.0', function(error) {
        if (error) {
...
function installSeries(meanieModules, cb) {
  ...
  //Get module
  var meanieModule = meanieModules.shift(),
      noInstalled = 0, self = this;

  //Run install
  install.call(this, meanieModule, function(error) {
    ...
    if (meanieModules.length > 0) {
      installSeries.call(self, meanieModules, cb);
      return;
    }
