...
function getVendorDir(packageName, withFile) {
  var vendorDir = path.join(meanie.project.vendorDir, packageName);
  if (withFile) {
    vendorDir = path.join(vendorDir, 'release', packageName + '.js');
  }
  return vendorDir.replace(meanie.project.dir + '/', '');
}
