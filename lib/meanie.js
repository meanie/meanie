...
/**
 * Determine project vendor dir
 */
function determineProjectVendorDir(meanie) {
  var vendorDir = 'client/vendor/';
  var bowerrcPath = path.join(meanie.project.dir, '.bowerrc');
  if (fs.existsSync(bowerrcPath)) {
    var bowerrc = jf.readFileSync(bowerrcPath);
    if (bowerrc.directory) {
      vendorDir = bowerrc.directory;
    }
  }
  return path.join(meanie.project.dir, vendorDir);
}
...
var meanie = module.exports = {
  ...
  load: function(env, cb) {
    ...
    this.project.vendorDir = determineProjectVendorDir(this);
